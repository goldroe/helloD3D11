...
WinMain(HINSTANCE instance, HINSTANCE prev_instance, LPSTR cmd_line, int cmd_show) {
    ...
    MeshData grid_mesh;
    generate_grid(160.0f, 160.0f, 50, 50, &grid_mesh);
    ...
    // Build vertex buffer
    Vertex *vertices = new Vertex[grid_mesh.vertex_count];
    for (UINT i = 0; i < grid_mesh.vertex_count; i++) {
        XMFLOAT3 P = grid_mesh.vertices[i];
        P.y = get_height(P.x, P.z);
        vertices[i].position = P;

        if (P.y < -10.0f) {
            vertices[i].color = XMFLOAT4(1.0f, 0.96f, 1.0f, 1.0f);
        }
        else if (P.y < 5.0f) {
    ...
    ID3D11Buffer *vertex_buffer = NULL;
    {
        ...
        vertex_buffer_desc.ByteWidth = grid_mesh.vertex_count * sizeof(Vertex);
    ...
    UINT indices_count = grid_mesh.index_count;

    ID3D11Buffer *index_buffer = NULL;
    {
        D3D11_BUFFER_DESC index_buffer_desc = {};
        index_buffer_desc.ByteWidth = grid_mesh.index_count * sizeof(uint32_t);
        index_buffer_desc.Usage = D3D11_USAGE_IMMUTABLE;
        index_buffer_desc.BindFlags = D3D11_BIND_INDEX_BUFFER;
        D3D11_SUBRESOURCE_DATA sr_data = {};
        sr_data.pSysMem = grid_mesh.indices;
