    ...
    switch (message) {
        ...
        case WM_MOUSEMOVE: {
            ...
            if (wparam & MK_RBUTTON) {
                float dx = 0.005f * (float)(x - last_mouse.x);
                float dy = 0.005f * (float)(y - last_mouse.y);
                radius += dx - dy;
                // radius = fclamp(radius, 3.0f, 15.0f);
            }
        ...
        case WM_SIZE: {
            ...
            XMMATRIX P = XMMatrixPerspectiveFovLH(XM_PIDIV2, aspect_ratio, 0.01f, 1000.0f);
...
WinMain(HINSTANCE instance, HINSTANCE prev_instance, LPSTR cmd_line, int cmd_show) {
    ...
    generate_cylinder(100.0f, 10.0f, 10.0f, 10, 20, &cylinder_mesh);
    ...
    HWND window = CreateWindowExA(0, window_class.lpszClassName, "HelloD3D11", WS_OVERLAPPEDWINDOW | WS_VISIBLE, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, instance, 0);
    assert(window);
    ...
    // Build vertex buffer
    Vertex *vertices = new Vertex[cylinder_mesh.vertices.size()];
    int i;
    for (i = 0; i < cylinder_mesh.vertices.size(); i++) {
        vertices[i].position = cylinder_mesh.vertices[i];
    ...
    ID3D11Buffer *vertex_buffer = NULL;
    {
        ...
        vertex_buffer_desc.ByteWidth = (UINT)cylinder_mesh.vertices.size() * sizeof(Vertex);
    ...
    uint32_t *indices = &cylinder_mesh.indices[0];
    UINT indices_count = (UINT)cylinder_mesh.indices.size();

    ID3D11Buffer *index_buffer = NULL;
    {
        D3D11_BUFFER_DESC index_buffer_desc = {};
        index_buffer_desc.ByteWidth = (UINT)cylinder_mesh.indices.size() * sizeof(uint32_t);
        index_buffer_desc.Usage = D3D11_USAGE_IMMUTABLE;
        index_buffer_desc.BindFlags = D3D11_BIND_INDEX_BUFFER;
        D3D11_SUBRESOURCE_DATA sr_data = {};
        sr_data.pSysMem = &cylinder_mesh.indices[0];
